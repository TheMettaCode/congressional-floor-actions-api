...
axios.get(senateSource.address)
  .then(senateResponse => {
    ...
    fs.writeFile("./outputs/floor_actions_senate.json", JSON.stringify(senateFloorActionsMap), (err) => {
...
axios.get(houseSource.address)
  .then(houseResponse => {
    ...
    fs.writeFile("./outputs/floor_actions_house.json", JSON.stringify(houseFloorActionsMap), (err) => {
      if (err) { console.log(err); }
    });
  });